...
| `F` | 第一人称行走：点击画面锁定鼠标，WASD 移动，Shift 加速，再按 `F` 返回环视。**墙、柱、长椅、关着的门都挡人**（拿 `src/grid.js` 那张射线网格当碰撞体，腰、头各打一条射线；撞上就分轴各试一次，贴着墙会自然滑过去）。只开便门时，只有便门那 0.85 m 宽能挤过去 |
| `P` | 设计与光面板：天气（晴 / 雨 / 雪，雨雪天自动关门）；门的小平面图；名堂预设（巴黎圣母院 / 沙特尔 / 亚眠 / 科隆，选中弹出年代、数据与故事的解说卡片）；时刻滑块（太阳自东向西，晨昏光色）；**镶玻方案**（沙特尔式 / 晚期银黄，换完自动重烘室内光照）；开间数 / 起拱高 / 尖拱曲率 / 塔高，改动即重建整座教堂 |
| `G` / `M` | 敲一记钟 / 静音（钟声为 Web Audio 非谐泛音合成；室外有风与鸟，入堂自动安静） |
| `K` | 开关门：五座门全开 ⇄ 全关。面板里的小平面图可以单独设每一座（关 → 便门 → 全开）；第一人称走到门前 4 m 内按 `E` 开关这一扇。开关都有木门吱呀声（关到位还有一记闷响），过教堂混响 |
| `C` | 剖面切换：完整 → 横剖 → 纵剖 |
...
`&weather=rain&doors=open&bake=0` 也可以（天气、门的状态、关掉光照烘焙）。
...
- **过梁 + 门楣 + 中柱**。真教堂的门远比门洞矮——门扇顶上横一道过梁，过梁以上的
  尖拱面是实心的门楣浮雕。中门净宽 3.8 m、门扇高 4.6 m（原来是一整片 6×13.65 m
  的板，比管风琴楼廊底面还高 5 m，向内开会直接撞进楼廊）。
- **两扇门板**，各带厚度、三道铁铰链带、门钉、门环；门轴朝室内（门龛层层内收，
  向外开会撞门龛）。
- **便门（wicket）**：中门与南耳堂门的左扇上挖一扇小门。巨门要几个人合力才推得开，
...
- **门龛上的人像**（按《最后的审判》的标准构图）：门楣正中基督坐在杏仁形的
  **曼多拉**里、两侧天使托举，最下面紧贴过梁的一带最小，是复活的死者；**中柱**上
  立一尊带头光的像（亚眠是"美丽的上帝"）；门龛侧壁一层一对**柱像**，脚下托座、
  头上华盖，越往里越小。人像是几个基本体拼的（锥台身子 + 收肩 + 球头 + 贴身双臂），
  这个工程不引外部模型，人像也只能这么长出来——远看轮廓对就够了。
- 门扇装在各自的枢轴组里（`src/doors.js`），`K` 键全开全关，面板里的小平面图
  点哪座切哪座。

**天气**（晴 / 雨 / 雪）改的不只是天色、雾和日照：**雨雪天五座门自动全关**——真教堂
就是这么干的，"礼拜开始后就得关门"在多雨多风的地方是常规操作。雨雪粒子在进堂之后
自动隐藏（屋顶底下当然看不见雨）。

门和漏雨检查器是连着的：

```bash
node tools/check-rain.mjs --wind=0,-0.8 --doors=closed   # 漏点 0
node tools/check-rain.mjs --wind=0,-0.8 --doors=open     # 落进室内 14 滴 = 2.2 m²
                                                          # 按 20 mm/h 算 ≈ 45 升/小时
```
...
手机与小内存设备走**低精度档**：射线 32→14、顶点缓存 0.25→0.45 m、射线网格 1.2→1.6 m、
Worker 只开 2 个。实测低精度档比全精度快 **1.9 倍**（22.6 s vs 43.5 s，单线程），而画面
差**最大 8/255、平均 0.06**——基本看不出来。判断"小设备"不能只看 `navigator.deviceMemory`：
iOS 的 Safari 根本不支持它，取不到值时不能当成大内存机器，所以屏宽、核数、指针类型
三样一起判。烘完存 IndexedDB，同一台机器只付一次。
