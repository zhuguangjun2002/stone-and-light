...
// 哪些网格要烘：主线程与 Worker 必须用同一套规则、同一个遍历顺序，
// 结果才能按下标对回去。
export function bakeableMeshes(scene) {
  const out = [];
  scene.traverse((o) => {
...
// 把算好的顶点色贴回网格。几何体是多处共用的，不逐网格克隆的话一处的结果会串到别处；
// 材质也是共用的，开 vertexColors 之前必须保证每一块用到该材质的网格都有 color 属性,
// 缺了着色器会当成黑色。
export function applyBakedColors(scene, colors) {
  const meshes = bakeableMeshes(scene);
  const mats = new Set();
  meshes.forEach((o, i) => {
    const c = colors[i];
    if (!c || c.length !== o.geometry.attributes.position.count * 3) return;
    if (o.geometry.userData.__baked !== o.uuid) {
      o.geometry = o.geometry.clone();
      o.geometry.userData.__baked = o.uuid;
    }
...
// 只算，不改场景：返回与 bakeableMeshes 同序的顶点色数组（没轮到自己烘的那些是 null）。
// opt.slice = {k, n} 时只烘 index % n === k 的网格，用来分给多个 Worker。
export function* bakeVertexLight(scene, opt = {}) {
  ...
  const colors = new Array(meshes.length).fill(null);

  // 同一处的顶点算一次就够：柱身、肋券这类构件在同一个位置往往叠着好几块网格的顶点，
  // 按"位置取整 + 法线朝向"做键缓存，能省掉一大半射线，而 0.25 m 的量化在顶点色
  // 插值之后看不出来。
  const cache = new Map();
  const QP = opt.cachePos ?? 0.25, QN = 4;      // 法线量化成 9×9×9 的格子
  ...
  meshes.forEach((o, mi) => {
    if (!slice || mi % slice.n === slice.k) total += o.geometry.attributes.position.count;
  });

  for (let mi = 0; mi < meshes.length; mi++) {
    const o = meshes[mi];
    if (slice && mi % slice.n !== slice.k) continue;
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      const key = `${Math.round(p.x / QP)},${Math.round(p.y / QP)},${Math.round(p.z / QP)},`
        + `${Math.round(nrm.x * QN)},${Math.round(nrm.y * QN)},${Math.round(nrm.z * QN)}`;
      const got = cache.get(key);
      if (got) {
        col[i * 3] = got[0]; col[i * 3 + 1] = got[1]; col[i * 3 + 2] = got[2];
        if (++done % chunk === 0) yield { done, total };
        continue;
      }
      // 快筛：先打 4 条散开的，全跑出去就当它完全露天——省下露天面的 7/8 射线。
      // 只在"一条都没挡住"时才敢这么断，所以不会把半遮的角落算亮。
      let open = true;
      for (let k = 0; k < 4 && open; k++) {
        const d = DIRS[Math.floor(k * DIRS.length / 4)];
        dir.copy(t1).multiplyScalar(d.x).addScaledVector(nrm, d.y).addScaledVector(t2, d.z).normalize();
        if (grid.hit(o3.x, o3.y, o3.z, dir.x, dir.y, dir.z)) open = false;
      }
      if (open) {
        col[i * 3] = sky.r; col[i * 3 + 1] = sky.g; col[i * 3 + 2] = sky.b;
        cache.set(key, [sky.r, sky.g, sky.b]);
        litVerts++;
        if (++done % chunk === 0) yield { done, total };
        continue;
      }
      ...
      cache.set(key, [col[i * 3], col[i * 3 + 1], col[i * 3 + 2]]);
      litVerts++;
      if (++done % chunk === 0) yield { done, total };
    }
    colors[mi] = col;
  ...
  return { colors, meshes: meshes.length, vertices: total, baked: litVerts, cached: cache.size };
}
